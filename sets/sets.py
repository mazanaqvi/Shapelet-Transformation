...
def main():
    ...
    all_shapelet_locations = np.load(os.path.join(inter_results, 'all_shapelet_locations.npy'), allow_pickle=True)
    all_shapelet_locations_test = np.load(os.path.join(inter_results, 'all_shapelet_locations_trth_test.npy'), allow_pickle=True)
    all_shapelets_class = np.load(os.path.join(inter_results, 'all_shapelets_class.npy'), allow_pickle=True).item()
    all_heat_maps = np.load(os.path.join(inter_results, 'all_heat_maps.npy'), allow_pickle=True).item()
    
    ts_length = X_train.shape[2]
    
    #Sort dimensions by their highest shapelet scores 
    all_shapelets_scores = np.load(os.path.join(inter_results, 'scores.npy'), allow_pickle=True)
