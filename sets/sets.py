...
def main():
    ...
    X_train = pd.read_csv(os.path.join('data', 'X_train.csv')).values
    y_train = pd.read_csv(os.path.join('data', 'y_train.csv')).values.ravel()
    X_test = pd.read_csv(os.path.join('data', 'X_test.csv')).values
    ...
    st_shapelets = pd.read_pickle(os.path.join(inter_results, 'shapelets.pkl'))
    all_shapelet_locations = np.loadtxt(os.path.join(inter_results, 'all_shapelet_locations.csv'), delimiter=',')
    all_shapelet_locations_test = np.loadtxt(os.path.join(inter_results, 'all_shapelet_locations_trth_test.csv'), delimiter=',')
    all_shapelets_class = pd.read_pickle(os.path.join(inter_results, 'all_shapelets_class.pkl'))
    all_heat_maps = pd.read_pickle(os.path.join(inter_results, 'all_heat_maps.pkl'))
    
    ts_length = X_train.shape[2]
    
    #Sort dimensions by their highest shapelet scores 
    all_shapelets_scores = np.loadtxt(os.path.join(inter_results, 'scores.csv'), delimiter=',')
    ...
    model = MLPClassifier(random_state=42, max_iter=100, hidden_layer_sizes=240,
          learning_rate_init=1e-3).fit(X_train.reshape((X_train.shape[0],-1)), y_train)
    ...
    for dim in range(X_test.shape[1]):    
        ...
        for label, (all_shapelets_class, shapelets_ranges) in enumerate(zip(
                list(all_shapelets_class.values()), list(shapelets_ranges.values()))):
        ...
        for c in np.unique(y_train):
            all_shapelets_ranges[c] = shapelets_ranges[c]
    ...
    for instance_idx in range(0, X_test.shape[0]):
        ...
        for target_c in set(np.unique(y_train)) - set([orig_c]):
            ...
            all_target_shapelets_class = all_shapelets_class[target_c]
